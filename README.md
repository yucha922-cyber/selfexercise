...
- **トップページ** (`/`): 会員ログイン画面。会員コードの入力欄と、
  「コードは施術時にお渡し／分からない方は担当セラピストへ」の案内を表示
  - **無料公開の3つ**: 院からのおすすめ（`clinic.ts` の `recommendedSlugs`）は
    ログインなしで動画・写真・手順まで閲覧可。ページ下部に会員案内を表示
...
> 無料公開する3つを差し替えるには、`src/config/clinic.ts` の `recommendedSlugs`
> を書き換えます。ライブラリの「院からのおすすめ」も同じ設定を使います。

## 会員様向けの機能

| 機能 | 内容 | 設定場所 |
|------|------|----------|
| 担当セラピストからの処方 | 会員コードごとに「今回のおすすめ3つ」とメッセージを指定。ライブラリ上部に表示 | `clinic.ts` の `prescription` / `message` |
| 会員コードの有効期限 | 期限を過ぎたコードはログイン不可。月替わりにすると継続通院者だけが閲覧可能 | `clinic.ts` の `expires` |
| セラピストのひとこと | セルフケアごとの個別コメント。該当ページ上部に表示 | `clinic.ts` の `notes` |
| 継続の記録 | 「今日やった」ボタンと連続日数・直近7日間の表示（端末内に保存） | 設定不要 |
| ホーム画面に追加 | Android はインストールボタン、iPhone は手順を案内 | 設定不要 |

> **個人情報の注意**: `clinic.ts` に書いた内容はサイトのソースから誰でも読めます。
> 患者様の本名・連絡先・症状などは書かないでください。呼び名は「Aさん」のように
> 個人が特定されない形にするか、空欄にします（空欄なら呼びかけは表示されません）。
...
肩回しやキャット&カウのように「開始→終了」ではなく **同じ動きをくり返す** 種目は、
`start` / `end` の代わりに **`step1.jpg` / `step2.jpg` / `step3.jpg`** と名前を付けます。
...
番号順に並び、写真の下に **「① → ② → ③ の順に、くり返し行いましょう。」** と自動で表示されます。
