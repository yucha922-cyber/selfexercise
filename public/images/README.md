...
## くり返す動作のとき（肩回し・キャット&カウなど）

「開始→終了」ではなく **同じ動きをくり返す** 種目は、`start` / `end` ではなく
**`step1` / `step2` / `step3`** という名前で置きます（最大8枚、2〜4枚が目安）。
...
- 番号順（①→②→③）に並び、写真の下に
  **「① → ② → ③ の順に、くり返し行いましょう。」** と自動で表示されます
- 3枚構成と同じように、ファイル名のうしろに `_` で説明を足せます
  - `step1_背中を丸める.jpg` → 「① 背中を丸める」
- `手順1.jpg` という日本語のファイル名でも同じように認識します

現在この形式を使っているのは `kata-scapula-roll`（肩回し）と
`youtsu-cat-stretch`（キャット&カウ）です。
